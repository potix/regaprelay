package gamepad

//
// XXX TODO
// I don't have a PlayStation 4.
// I want a PlayStation 5 rather than a PlayStation 4.
// Perhaps I will never implement this in my lifetime.
//

import(
        "fmt"
        "log"
        "time"
	"github.com/potix/regaprelay/backend/setup"
)

type PS4Con struct  {
	BaseBackend
	setupParams *setup.UsbGadgetHidSetupParams
	verbose bool
}

func (p *PS4Con) Setup() error {
        err := setup.UsbGadgetHidSetup(p.setupParams)
        if err != nill {
                return fmt.Errorf("setup error in ps4con: %w", err)
        }
        return nil
}

func (p *PS4Con) Start() error {
        err := UsbGadgetHidEnable(p.setupParams)
        if err != nill {
                return fmt.Errorf("can not enable usb gadget hid device in nsprocon: %w", err)
        }
}

func (p *PS4Con) Stop() {
        err := UsbGadgetHidDisable(p.setupParams)
        if err != nill {
                log.Printf("can not enable usb gadget hid device in nsprocon: %v", err)
        }
}

func (p *PS4Con) UpdateState(state *handler.GamepadState) error {
}

func (p *PS4Con) Press(buttons ...ButtonName) error {
}

func (p *PS4Con) Release(buttons ...ButtonName) error {
}

func (p *PS4Con) Push(buttons ...ButtonName, duration time.MilliSecond) error {
}

func (p *PS4Con) Repeat(buttons ...ButtonName, interval time.MilliSecond, duration time.MilliSecond) error {
}

func (p *PS4Con) StickL(xSir XDirection, xPower float64, yDir YDirection, yPower float64, duration time.MilliSecond) error {
}

func (p *PS4Con) StickR(xSir XDirection, xPower float64, yDir YDirection, yPower float64, duration time.MilliSecond) error {
}

func (p *PS4Con) StickRotationLeft(lapTime time.MilliSecond, power float64, duration time.MilliSecond) error {
}

func (p *PS4Con) StickRotationRight(lapTime time.MilliSecond, power float64, duration time.MilliSecond) error {
}

func NewPs4Con(verbose bool) *PS4Con {
        setupParams := &setup.UsbGadgetHidSetupParams{
                ConfigsHome:     "/sys/kernel/config",
                GadgetName:      "ps4con",
                IdProduct:       "0x054c",
                IdVendor:        "0x05c4",
                BcdDevice:       "0x0100",
                BcdUsb:          "0x0200",
                BMaxPacketSize0: "64",
                BDeviceProtocol: "0",
                BDeviceSubClass: "0",
                BDeviceClass:    "0",
                StringsLang:     "0x409",
                ISerial:         "0",
                IProduct:        "Wireless Controller",
                IManufacture:    "Sony Computer Entertainment",
                ConfigName:      "c",
                ConfigNumber:    "1",
                ConfigString:    "Play Station 4 Controller",
                BmAttributes:    "0xc0",
                MaxPower:        "500mA",
                FunctionName:    "hid",
                InstanceName:    "usb0",
                Protocol:        "0",
                Subclass:        "0",
                ReportLength:    "499",
                ReportDesc:      "",
        }
	return &PS4Con{
		setupPrams: setupParams,
		verbose: verbose,
	}
}
